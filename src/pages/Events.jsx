...
import {addNewEvent, deleteNewEvent, updateNewEvent} from "../store/slices/eventsSlice";
...
const Events = () => {
  ...
  return (
    <Container fluid="xl">
      <CustomButton variant="primary" version="add" onClick={() => {
        handleShowModal();
        removeRequireConfirm();
      }}>
        <BsPlusLg/>
      ...
      <ModalForm
        title={
          requireConfirm && deleteMode ? "Do You confirm deleting event?" :
            requireConfirm && !itemEdit.edit ? "Do You confirm adding new event?" :
              !requireConfirm && !itemEdit.edit ? "Add a new Event" :
                requireConfirm && itemEdit.edit ? "Do You confirm updating event?" :
                  "Update event"
        }
        form={
          requireConfirm && deleteMode ?
            <ConfirmForm handleConfirmation={e => {
              e.preventDefault();
